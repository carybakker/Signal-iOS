...
@objc
public enum PaymentPreviewError: Int, Error {
...
// MARK: - OWSPaymentPreviewDraft

// This contains the info for a link preview "draft".
public class OWSPaymentPreviewDraft: NSObject {
    @objc
    public var url: URL
    ...
    public func displayDomain() -> String? {
        return OWSPaymentPreviewManager.displayDomain(forUrl: urlString)
    }
}

// MARK: - OWSPaymentPreview

@objc
public class OWSPaymentPreview: MTLModel {
    ...
    public class func isNoPreviewError(_ error: Error) -> Bool {
        guard let error = error as? PaymentPreviewError else {
            return false
        }
    ...
    @objc
    public class func buildValidatedPaymentPreview(dataMessage: SSKProtoDataMessage,
                                                body: String?,
                                                transaction: SDSAnyWriteTransaction) throws -> OWSPaymentPreview {
        guard let previewProto = dataMessage.preview.first else {
            throw PaymentPreviewError.noPreview
        }
        guard dataMessage.attachments.count < 1 else {
            Logger.error("Discarding link preview; message has attachments.")
            throw PaymentPreviewError.invalidPreview
        }
        let urlString = previewProto.url

        guard let url = URL(string: urlString), url.isPermittedPaymentPreviewUrl() else {
        ...
        let linkPreview = OWSPaymentPreview(urlString: urlString, title: title, imageAttachmentId: imageAttachmentId)
    ...
    @objc
    public class func buildValidatedPaymentPreview(fromInfo info: OWSPaymentPreviewDraft,
                                                transaction: SDSAnyWriteTransaction) throws -> OWSPaymentPreview {
        let imageAttachmentId = OWSPaymentPreview.saveAttachmentIfPossible(imageData: info.imageData,
                                                                        imageMimeType: info.imageMimeType,
                                                                        transaction: transaction)

        let linkPreview = OWSPaymentPreview(urlString: info.urlString, title: info.title, imageAttachmentId: imageAttachmentId)
...
@objc
public class OWSPaymentPreviewManager: NSObject, Dependencies {
    ...
    public func findFirstValidUrl(in searchString: String) -> URL? {
        ...
        return allMatches.first(where: {
            ...
            return parsedUrl.isPermittedPaymentPreviewUrl(parsedFrom: matchedString)
        })?.url
    }

    @objc(fetchPaymentPreviewForUrl:)
    @available(swift, obsoleted: 1.0)
    public func fetchPaymentPreview(for url: URL) -> AnyPromise {
        let promise: Promise<OWSPaymentPreviewDraft> = fetchPaymentPreview(for: url)
        return AnyPromise(promise)
    }

    public func fetchPaymentPreview(for url: URL) -> Promise<OWSPaymentPreviewDraft> {
        return firstly(on: Self.workQueue) { () -> Promise<OWSPaymentPreviewDraft> in
            return self.fetchPaymentPreview(forGenericUrl: url)
        }.map(on: Self.workQueue) { (linkPreviewDraft) -> OWSPaymentPreviewDraft in
    ...
    private func fetchPaymentPreview(forGenericUrl url: URL) -> Promise<OWSPaymentPreviewDraft> {
        firstly(on: Self.workQueue) { () -> Promise<(URL, String)> in
            self.fetchStringResource(from: url)

        }.then(on: Self.workQueue) { (respondingUrl, rawHTML) -> Promise<OWSPaymentPreviewDraft> in
            ...
            let draft = OWSPaymentPreviewDraft(url: url, title: normalizedTitle)
            ...
            draft.date = content.dateForPaymentPreview
            ...
            return firstly(on: Self.workQueue) { () -> Promise<Data> in
            ...
            }.map(on: Self.workQueue) { (previewThumbnail: PreviewThumbnail?) -> OWSPaymentPreviewDraft in
                ...
                return draft
            }.recover(on: Self.workQueue) { (_) -> Promise<OWSPaymentPreviewDraft> in
                return Promise.value(draft)
            }
        }
    }
    ...
    private func buildOWSURLSession() -> OWSURLSession {
        ...
        urlSession.customRedirectHandler = { request in
            guard request.url?.isPermittedPaymentPreviewUrl() == true else {
                return nil
            }
            return request
        }
    ...
    func fetchStringResource(from url: URL) -> Promise<(URL, String)> {
        firstly(on: Self.workQueue) { () -> Promise<(HTTPResponse)> in
            ...
            guard statusCode >= 200 && statusCode < 300 else {
                ...
                throw PaymentPreviewError.fetchFailure
            }
    ...
    private func linkPreviewDraft(forStickerShare url: URL) -> Promise<OWSPaymentPreviewDraft> {
        Logger.verbose("url: \(url)")

        guard let stickerPackInfo = StickerPackInfo.parseStickerPackShare(url) else {
            Logger.error("Could not parse url.")
            return Promise(error: PaymentPreviewError.invalidPreview)
        }
        ...
        return firstly(on: Self.workQueue) {
        ...
        }.then(on: Self.workQueue) { (stickerPack: StickerPack) -> Promise<OWSPaymentPreviewDraft> in
            ...
            return firstly { () -> Promise<URL> in
                ...
                guard let previewThumbnail = previewThumbnail else {
                    return OWSPaymentPreviewDraft(url: url,
...
fileprivate extension URL {
    ...
    /// be ASCII.
    func isPermittedPaymentPreviewUrl(parsedFrom sourceString: String? = nil) -> Bool {
...
fileprivate extension HTMLMetadata {
    var dateForPaymentPreview: Date? {
...
fileprivate extension OWSPaymentPreviewManager {
    @objc
